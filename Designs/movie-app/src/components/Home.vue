      ...
      <nav class="navbar navbar-dark bg-dark">
        <div class="container-fluid">
          ...
          <form class="d-flex" role="search">
            ...
            <button class="btn btn-dark" type="submit" @click.prevent="searchMovies">Search</button>
            <button class="btn btn-secondary ms-2" @click.prevent="resetearSearch">Reset</button>
          </form>
        </div>
      </nav>
      <div v-for="(movies, category) in categorizedMovies" :key="category">
        ...
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
          <div v-for="movie in movies" :key="movie.id" class="col">
            <div :id="'movie-card-' + movie.id" class="card shadow-sm h-100">
